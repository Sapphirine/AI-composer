...
NOTTINGHAM_MELODY_MAX = 128
NOTTINGHAM_MELODY_MIN = 0
...
def parse_nottingham_directory(input_dir, time_step, verbose=False):
    ...
    for f in files:
        try:
            sequences.append(parse_nottingham_to_sequence(f,time_step=time_step,verbose=verbose))
        except:
...
def parse_nottingham_to_sequence(input_filename, time_step, verbose=False):
    ...
    time_step = pattern.resolution/4
